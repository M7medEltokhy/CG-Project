...
# تحميل الخلفية بعد تهيئة النافذة
...
class Ghost:
    ...
    def update(self, pac_pos, dt, level):
        chase_factor = 0.55 + 0.05 * max(0, level - 1)
...
class GameState:
    ...
    def spawn_ghosts_for_level(self):
        self.ghosts.clear()
        num_ghosts = min(4, 1 + (self.level - 1) // 1)
...
def main():
    ...
    while running:
        ...
        pac_surface.set_alpha(max(0, min(255, int(255 * flash))))
